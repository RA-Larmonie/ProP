...
try {
        ...
        /* else if (!preg_match("/^[0-9]{3}$/", $creditCardSecurityCode)) {
			...
			$sql = "SELECT Type_Of_Ticket FROM ticket WHERE UserID=? AND Type_Of_Ticket='1D'";
			...
			exit();
				...
				if($resultCheck > 0){
					...
					if($amountOfTicket == 2){
							...
							$sql = "UPDATE visitor SET isPaid=? WHERE UserID=?";					
					...
					$sql = "UPDATE visitor SET isPaid=? WHERE UserID=$user_id";
						$stmt = $conn->prepare($sql);
						if(!$stmt){
							header("Location: ../pages/tickets/Tickets.php?error=sqlerror");
							exit();
